...
const categories = [
  { name: 'إلكترونيات', image: '/category-electronics.jpg', icon: Smartphone, count: '+١٥,٠٠٠ منتج' },
...
export default function CategoriesGrid() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo('.cat-card', { y: 30, opacity: 0 }, {
        y: 0, opacity: 1, duration: 0.4, ease: 'expo.out', stagger: 0.1,
        scrollTrigger: { trigger: '.cat-grid', start: 'top 85%', toggleActions: 'play none none none' },
      });
    }, sectionRef);
    return () => ctx.revert();
  }, []);
            ...
            return (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 text-center">
                  <Icon className="w-6 h-6 text-[#D4A853] mx-auto mb-2" strokeWidth={1.5} />
                  <h3 className="text-white font-cairo font-bold text-sm md:text-base mb-1">
                    {cat.name}
                  </h3>
                  <span className="text-[#AAAAAA] text-xs font-cairo">{cat.count}</span>
                </div>
              </Link>
            );
          })}
