...
const customerSteps = [
	...
	{ icon: Heart, title: 'قارن واختر', desc: 'قارن الأسعار والتقييمات', color: '#2563EB' },
	{
		icon: ShoppingCart,
		title: 'اطلب بثقة',
		desc: 'ادفع عند الاستلام أو إلكترونياً',
		color: '#2563EB',
	},
...
function StepColumn({
	...
	return (
		<div className="relative">
			<h3
				className="text-lg font-cairo font-bold mb-6 text-center"
			...
			<div className="relative space-y-6">
				...
				<div
					...
					style={{
						background: `linear-gradient(to bottom, ${accentColor}40, ${accentColor})`,
					}}
				/>
				{steps.map((step, i) => {
					...
					return (
						<div
							key={i}
							className={`hw-step flex items-start gap-4 relative ${label === 'للعملاء' ? 'md:flex-row-reverse' : ''}`}
						>
							<div
							...
							<div className="flex-1 pt-1">
								<h4 className="font-cairo font-bold text-[#111111] text-sm md:text-base mb-1">
									{step.title}
								</h4>
								<p className="text-[#6B6B6B] text-xs md:text-sm font-cairo leading-relaxed">
									{step.desc}
								</p>
							</div>
						</div>
