...
| **Database**          | ✅ One external PostgreSQL 17 server, database `noufex_db`. **29 tables** (26 application + 3 system: `rate_limit_buckets`, `search_logs`, `schema_migrations`) + 60+ indexes + 9 triggers + 4 views + 3 roles. App connects as `noufex_app` (least privilege). | [`docs/database.md`](database.md)                          |
| **DB setup**          | ✅ One-time CLI: `npm run db:setup` applies schema + 9 migrations + seed. Migrations table deduplicated (12 → 9 rows, descriptive version keys).                                  | `scripts/db-setup.cjs`                                     |
| **API server**        | ✅ Express 5 + `pg`. Scrypt hashing, zod validation, CORS allow-list, DB-backed rate limiter on `/api/auth/*` + per-endpoint rate limits on `/api/auth/2fa/*` (5 buckets, 8 endpoints), SPA fallback. Source → esbuild ESM bundle at build time. | `app/server/index.ts` (source) / `app/server/index.js` (build artefact) |
| **Frontend**          | ✅ React 19 + TS strict + Vite 7 + Tailwind + shadcn/ui. 49 routes. AR/EN/ZH i18n.                                                                                                     | `app/src/App.tsx`                                          |
| **Tests**             | ✅ Vitest 4 + supertest. **464/464 passing** across 37 test files. PgDb is mocked globally so tests run without a live database.                                                   | [`docs/testing.md`](testing.md)                            |
