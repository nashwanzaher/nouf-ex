...
--  Audit hygiene notes
--  -------------------
--  - DO $$ ... END $$ blocks use information_schema.columns so the
--    migration is idempotent: re-running it is a no-op once the
--    column is present.
--  - All column additions use IF NOT EXISTS-equivalent semantics,
--    so no migration table is needed for this.
--  - ANALYZE on each affected table at the end so the planner
--    picks up the new columns / indexes immediately.
--  - Soft-delete partial indexes use WHERE deleted_at IS NULL to
--    mimic the idx_products_active partial-index pattern already
--    used elsewhere in the schema - keeping the live-rows-only
--    performance characteristic consistent.
--
--  IMPORTANT (do-not-break): each DO block in this file is a single
--  PL/pgSQL unit terminated by END $$. The token END IF; must be on
--  one line; the token END $$; must be on one line. A
--  formatter or pre-commit hook that line-wraps these tokens
--  will corrupt the migration and psql will reject it with a
--  syntax error. CI should grep for 'END IF;' and 'END $$;' on
--  one line as a guard rail.
-- =====================================================================
