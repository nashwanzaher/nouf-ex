...
export default function SubscriptionTiers() {
	...
	return (
		<section
			ref={sectionRef}
			className="bg-[#1A1612] py-16 md:py-24 lg:py-[100px] relative overflow-hidden"
		>
			{/* Noise texture */}
			<div
			...
			<div className="max-w-container mx-auto container-pad relative z-10">
				...
				<div className="st-header text-center mb-10 md:mb-14">
					...
					<div className="flex items-center justify-center gap-3">
						<span
							className={`text-sm font-cairo font-semibold ${!yearly ? 'text-white' : 'text-[#AAAAAA]'}`}
						>
							شهري
						</span>
						<Switch
				...
				<div className="st-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-6">
					{tiers.map((tier) => (
						...
						>
							...
							<h3
								className={`font-cairo font-bold text-lg mb-2 ${tier.featured ? 'text-[#1A1612]' : 'text-white'}`}
							>
								{tier.name}
							</h3>
							...
							<div className="mb-5">
								<span
									className={`font-mono font-bold text-2xl ${tier.featured ? 'text-[#D4A853]' : 'text-[#D4A853]'}`}
								>
									{yearly ? tier.yearlyPrice : tier.monthlyPrice}
								</span>
								{tier.monthlyPrice !== 'Custom' && (
									<span
										className={`text-xs font-cairo mr-1 ${tier.featured ? 'text-[#6B6B6B]' : 'text-[#AAAAAA]'}`}
									>
								...
								{tier.features.map((feature) => (
									<li key={feature.text} className="flex items-center gap-2">
										{feature.included ? (
											...
											<X
												className="w-4 h-4 text-[#AAAAAA] shrink-0"
												strokeWidth={2}
											/>
										)}
										<span
											className={`text-xs md:text-sm font-cairo ${
												feature.included
													? tier.featured
														? 'text-[#111111]'
														: 'text-[#ddd]'
