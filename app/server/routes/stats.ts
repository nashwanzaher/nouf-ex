...
statsRouter.get('/home', async (_req: Request, res: Response) => {
	try {
		...
		const productsCount = pick(
			await db.prepare('SELECT COUNT(*) as count FROM products WHERE is_active = TRUE').get(),
		);
		const storesCount = pick(
			await db.prepare('SELECT COUNT(*) as count FROM stores WHERE is_active = TRUE').get(),
		);
		const ordersCount = pick(
			await db.prepare("SELECT COUNT(*) as count FROM orders WHERE status != 'cancelled'").get(),
		);
		const usersCount = pick(
			await db.prepare('SELECT COUNT(*) as count FROM users WHERE deleted_at IS NULL').get(),
		);
