...
export default function MobileAppCTA() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.fromTo('.app-content', { x: 40, opacity: 0 }, {
        x: 0, opacity: 1, duration: 0.6, ease: 'expo.out',
        scrollTrigger: { trigger: sectionRef.current, start: 'top 75%' },
      });
      gsap.fromTo('.app-mockup', { x: -40, opacity: 0, rotate: 0 }, {
        x: 0, opacity: 1, rotate: -5, duration: 0.8, ease: 'expo.out',
        scrollTrigger: { trigger: sectionRef.current, start: 'top 75%' },
      });
    }, sectionRef);
    return () => ctx.revert();
  ...
  return (
    <section ref={sectionRef} className="bg-[#1A1612] py-16 md:py-24 lg:py-[100px] overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <div className="app-content text-center lg:text-right">
            ...
            <p className="text-[#AAAAAA] text-sm md:text-lg font-cairo leading-relaxed mb-8 max-w-md mx-auto lg:mr-0 lg:ml-auto">
              حمّل تطبيق نوف-إكس واستمتع بتجربة تسوق سلسة. تتبع طلباتك، تواصل مع التجار، واستلم إشعارات فورية.
            </p>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-3 mb-6">
              <button className="h-12 px-5 bg-[#111111] border border-white/20 rounded-xl flex items-center gap-3 hover:border-[#D4A853]/50 transition-colors w-full sm:w-auto justify-center">
                ...
                <div className="text-left">
                  <div className="text-[9px] text-[#AAAAAA] font-cairo leading-none">Download on the</div>
                  <div className="text-sm text-white font-cairo font-bold leading-tight">App Store</div>
                </div>
              </button>
              <button className="h-12 px-5 bg-[#111111] border border-white/20 rounded-xl flex items-center gap-3 hover:border-[#D4A853]/50 transition-colors w-full sm:w-auto justify-center">
                <svg className="w-6 h-6 text-white" viewBox="0 0 24 24" fill="currentColor">
                ...
                <div className="text-left">
                  <div className="text-[9px] text-[#AAAAAA] font-cairo leading-none">GET IT ON</div>
                  <div className="text-sm text-white font-cairo font-bold leading-tight">Google Play</div>
                </div>
              </button>
            </div>
