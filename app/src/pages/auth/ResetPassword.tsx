...
function PasswordStrength({ password }: { password: string }) {
  ...
  const textColors = ['text-[#EF4444]', 'text-[#F59E0B]', 'text-[#D4A853]', 'text-[#10B981]', 'text-[#2563EB]'];
    ...
    return (
      <AuthLayout showBackLink={false}>
        <div className="animate-in zoom-in-95 duration-300 text-center">
          ...
          <h2 className="text-2xl font-amiri font-bold text-[#1A1612] mb-3">تم تغيير كلمة المرور!</h2>
  ...
  return (
    <AuthLayout backLink="/auth/forgot-password" backLabel="العودة">
      <div className="animate-in slide-in-from-left-4 duration-500">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-amiri font-bold text-[#1A1612] mb-2">تعيين كلمة مرور جديدة</h1>
          <p className="text-[#6B6B6B] font-cairo text-sm">أدخل كلمة المرور الجديدة أدناه</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-5">
          {/* New Password */}
          <div>
            <Label className="font-cairo text-sm text-[#111111]">كلمة المرور الجديدة</Label>
            <div className="relative mt-1.5">
              <Lock className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-[#AAAAAA]" strokeWidth={1.5} />
              <Input
            ...
            {errors.password && <p className="text-[#EF4444] text-xs font-cairo mt-1">{errors.password}</p>}
          </div>

          {/* Confirm Password */}
          <div>
            <Label className="font-cairo text-sm text-[#111111]">تأكيد كلمة المرور</Label>
            <div className="relative mt-1.5">
              <Lock className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-[#AAAAAA]" strokeWidth={1.5} />
              <Input
            ...
            {errors.confirmPassword && <p className="text-[#EF4444] text-xs font-cairo mt-1">{errors.confirmPassword}</p>}
          </div>
