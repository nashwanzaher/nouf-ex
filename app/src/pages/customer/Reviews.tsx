...
const pendingReviews: PendingReview[] = [
  { id: 1, productName: 'سماعات لاسلكية فاخرة مع إلغاء الضوضاء', purchaseDate: '١٥ يونيو ٢٠٢٥', merchant: 'إلكترونيات اليمن' },
  { id: 2, productName: 'ساعة ذكية رياضية مقاومة للماء', purchaseDate: '١٠ يونيو ٢٠٢٥', merchant: 'تك ستور' },
  { id: 3, productName: 'عسل يمني كريمي درجة أولى', purchaseDate: '٥ يونيو ٢٠٢٥', merchant: 'منتجات يمنية' },
];
...
export default function Reviews() {
  ...
  const filteredSubmitted = reviewTab === 'all'
    ? submitted
    : reviewTab === 'positive'
    ...
    <div className="min-h-[100dvh] bg-[#F8F8F8]" dir="rtl">
            ...
            <div className="space-y-3">
              {pendingReviews.length === 0 ? (
                <div className="bg-white rounded-2xl p-12 text-center shadow-sm">
                  <CheckCircle className="w-16 h-16 text-[#10B981] mx-auto mb-4" strokeWidth={1.5} />
                  <h3 className="text-xl font-amiri font-bold text-[#1A1612] mb-2">لا يوجد منتجات بانتظار التقييم</h3>
                  <p className="text-[#6B6B6B] font-cairo text-sm">لقد قمت بتقييم جميع منتجاتك</p>
                </div>
              ) : (
                pendingReviews.map((item) => (
                  <div key={item.id} className="bg-white rounded-2xl p-5 shadow-sm flex flex-col sm:flex-row sm:items-center gap-4">
                    <div className="w-16 h-16 rounded-xl bg-[#F8F8F8] flex items-center justify-center shrink-0">
                      <Package className="w-8 h-8 text-[#AAAAAA]" strokeWidth={1} />
                    </div>
                    <div className="flex-1">
                      <h3 className="font-cairo font-semibold text-sm text-[#111111]">{item.productName}</h3>
          ...
          {activeTab === 'submitted' && (
            <div className="space-y-4">
              ...
              {filteredSubmitted.length === 0 ? (
                <div className="bg-white rounded-2xl p-12 text-center shadow-sm">
                  <Star className="w-16 h-16 text-[#AAAAAA] mx-auto mb-4" strokeWidth={1} />
                  <h3 className="text-xl font-amiri font-bold text-[#1A1612] mb-2">لا توجد تقييمات</h3>
                </div>
              ) : (
                    ...
                    <div className="p-5">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <h3 className="font-cairo font-semibold text-sm text-[#111111]">{review.productName}</h3>
                            <div className="flex items-center gap-1 mt-1">
                              ...
                              <span className="text-xs text-[#6B6B6B] font-cairo mr-2">{review.date}</span>
                            </div>
                          </div>
                        </div>
                      ...
                      {review.photos.length > 0 && (
                        <div className="flex gap-2 mt-3">
                          {review.photos.map((photo, idx) => (
                            <div key={idx} className="w-20 h-20 rounded-lg overflow-hidden bg-[#F8F8F8]">
                      ...
                      {review.merchantReply && (
                        <div className="mt-4 bg-[#F3EDE4] rounded-xl p-4 border-r-3 border-r-[#D4A853]">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-xs font-cairo font-semibold text-[#D4A853]">رد التاجر</span>
                          </div>
                          <p className="text-sm text-[#6B6B6B] font-cairo">{review.merchantReply}</p>
                        </div>
                      )}
                    </div>
                  </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-lg rounded-2xl" dir="rtl">
          <DialogHeader>
            <DialogTitle className="font-amiri text-xl text-[#1A1612]">كتابة تقييم</DialogTitle>
          </DialogHeader>
          {selectedPending && (
            <div className="space-y-5 pt-2">
              <div className="flex items-center gap-3 bg-[#F8F8F8] rounded-xl p-3">
                ...
                <div>
                  <p className="font-cairo font-semibold text-sm text-[#111111]">{selectedPending.productName}</p>
                  <p className="text-xs text-[#6B6B6B] font-cairo">{selectedPending.merchant}</p>
                </div>
              </div>

              <div>
                <label className="block text-sm font-cairo font-semibold text-[#111111] mb-2">التقييم</label>
                <StarRatingInput rating={rating} onRate={setRating} />
              </div>

              <div>
                <label className="block text-sm font-cairo font-semibold text-[#111111] mb-2">تعليقك</label>
                <Textarea
                  value={reviewText}
                ...
                />
                <p className="text-[10px] text-[#AAAAAA] font-cairo mt-1">{reviewText.length} حرف</p>
              </div>

              <div>
                <label className="block text-sm font-cairo font-semibold text-[#111111] mb-2">الصور (اختياري)</label>
                <div className="flex gap-2 flex-wrap">
                  ...
                  {photos.length < 3 && (
                    ...
                    >
                      ...
                      <span className="text-[10px] font-cairo mt-1">إضافة</span>
                    </button>
                  )}
                  <input
                    ref={fileInputRef}
