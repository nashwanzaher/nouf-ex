...
const initialNotifications: Notification[] = [
  ...
  },
  {
    ...
    message: 'استخدم كود NOUF25 واحصل على خصم ٢٥% على جميع منتجات الإلكترونيات. العرض ساري حتى نهاية الأسبوع.',
    time: 'منذ ساعتين',
    read: false,
    link: '/deals',
  },
  ...
  },
  {
    ...
    message: 'لقد حصلت على ١٠٠ نقطة نوف بمناسبة عيد ميلاد المنصة. استخدمها في مشترياتك القادمة.',
    time: 'منذ ٣ أيام',
    read: true,
  },
  {
        ...
        <div className="p-6 max-w-3xl mx-auto space-y-4">
          ...
          {filtered.length === 0 ? (
            <div className="bg-white rounded-2xl p-12 text-center shadow-sm">
              ...
              <h3 className="text-xl font-amiri font-bold text-[#1A1612] mb-2">لا إشعارات جديدة</h3>
              <p className="text-[#6B6B6B] font-cairo text-sm">ستظهر إشعاراتك الجديدة هنا</p>
            </div>
          ) : (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <p className={`font-cairo text-sm ${!n.read ? 'font-semibold' : 'font-medium'} text-[#111111]`}>
                          {n.title}
                        </p>
                        {!n.read && (
                      ...
                      <p className="text-xs text-[#6B6B6B] font-cairo leading-relaxed line-clamp-2">{n.message}</p>
                      <p className="text-[10px] text-[#AAAAAA] font-cairo mt-2">{n.time}</p>
                    </div>
                    <ChevronLeft className="w-4 h-4 text-[#AAAAAA] shrink-0 self-center" strokeWidth={1.5} />
                  </div>
                );
              })}
            </div>
