...
export default function LiveCommerce() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.fromTo('.lc-video', { scale: 0.95, opacity: 0 }, {
        scale: 1, opacity: 1, duration: 0.6, ease: 'expo.out',
        scrollTrigger: { trigger: sectionRef.current, start: 'top 75%' },
      });
      gsap.fromTo('.lc-content', { x: 40, opacity: 0 }, {
        x: 0, opacity: 1, duration: 0.5, ease: 'expo.out', stagger: 0.1,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <div className="lc-video relative rounded-3xl overflow-hidden aspect-video shadow-2xl">
            ...
            <div className="absolute bottom-4 right-4 glass-dark px-3 py-1.5 rounded-lg">
              <span className="text-white text-xs font-cairo font-semibold">👁 ١,٢٤٥ مشاهد</span>
            </div>
            ...
            >
              <div className="w-20 h-20 rounded-full glass-light flex items-center justify-center animate-pulse-glow group-hover:scale-110 transition-transform">
                <Play className="w-8 h-8 text-[#1A1612] mr-1" strokeWidth={1.5} fill="#1A1612" />
              </div>
            </button>
          </div>
          ...
          <div>
            ...
            <p className="lc-content text-[#6B6B6B] text-sm md:text-lg font-cairo leading-relaxed mb-6 max-w-md">
              شاهد التجار يعرضون منتجاتهم مباشرة، اسألهم، تفاعل، واشتري فوراً. تجربة تسوق جديدة تجمعك بالتاجر مباشرة.
            </p>
                ...
                return (
                  <li key={f.text} className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-lg bg-[#1A1612]/5 flex items-center justify-center">
                      <Icon className="w-4 h-4 text-[#D4A853]" strokeWidth={1.5} />
                    </div>
                    <span className="text-[#111111] text-sm md:text-base font-cairo">{f.text}</span>
                  </li>
                );
              })}
            </ul>
